...
function getIntegrityOptions({ integrityOptions }) {
    ...
    const ignoreOption = Array.isArray(integrityOptions.ignore)
        ...
        : [];
    return Object.assign(Object.assign({}, integrityOptions), { ignore: [...ignore, ...ignoreOption], absolute: true });
}
// @ts-ignore
function ensureOptionsDefaults(options) {
    const { suite = 'Storyshots', asyncJest, storyNameRegex, storyKindRegex, renderer, serializer, snapshotSerializers, stories2snapsConverter = defaultStories2SnapsConverter, test: testMethod = test_bodies_1.snapshotWithOptions({ renderer, serializer }), } = options;
